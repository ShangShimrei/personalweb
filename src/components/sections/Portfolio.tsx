...
const PortfolioItem: React.FC<{ project: Project; index: number }> = ({ project, index }) => {
  ...
  return (
    <div className={`group cursor-pointer reveal stagger-${(index % 2) + 1}`}>
      <div className="aspect-[4/3] overflow-hidden bg-zinc-200 mb-4 shadow-sm relative rounded-none">
        
        {/* Loading Skeleton */}
        {isLoading && (
          <div className="absolute inset-0 z-20 bg-zinc-100 animate-pulse flex items-center justify-center">
            <ImageIcon className="w-8 h-8 text-zinc-300/50" />
          </div>
        )}
